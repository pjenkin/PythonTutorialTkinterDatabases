...
def with_connection(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        return func(*args, **kwargs)    # https://realpython.com/inner-functions-what-are-they-good-for/ - ops, commits &c
...
# @with_connection # NB decorator seemingly not working so well with PostGreSQL
def create_table():
    connection = psycopg2.connect("dbname='database1' user='postgres' password='password' host='localhost' port='5432'")
    ...
    # cursor.execute('CREATE TABLE store(item TEXT, quantity INTEGER, price REAL)')       # REAL/float
    print('table should have been made')
    connection.commit()
    connection.close()
...
# abbreviated version of 'update' below, using custom decorator - NB renamed only for PostGreSQL where seemed not working so well


@with_connection
def update_with_decorator(item, quantity, price):
    sql_string = f'UPDATE store SET quantity={quantity}, price={price} WHERE item=\'{item}\'' # NB f prefix
    print(sql_string)
    cursor.execute(sql_string)    # NB f prefix
    connection.commit()
